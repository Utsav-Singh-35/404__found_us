document.addEventListener('DOMContentLoaded', function() {
    ...
    function addMessage(text, type, messageId = null) {
        ...
        if (type === 'bot') {
            // Check if this is a fact-check result
            // Look for: "Fact-Check Result", confidence emoji (🟢🟡🔴), or "Confidence:" text
            const isFactCheck = text.includes('Fact-Check Result') || 
                               text.includes('Confidence:') ||
                               text.includes('🟢') || 
                               text.includes('🟡') || 
                               text.includes('🔴') ||
                               /\d+%/.test(text);  // Contains percentage
            
            let actionButtons = '';
            if (isFactCheck && messageId) {
                actionButtons = `
                    <div class="message-actions">
                        <button class="publish-btn" onclick="publishFactCheck('${messageId}', this)" title="Publish and email this fact-check report">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        <button class="download-btn" onclick="downloadFactCheck('${messageId}', \`${escapeHtml(text).replace(/`/g, '\\`')}\`)" title="Download fact-check report">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    // Download fact-check function
    window.downloadFactCheck = function(messageId, content) {
        try {
            // Create a formatted text document
            const timestamp = new Date().toLocaleString();
            const reportContent = `
SATYAMATRIX FACT-CHECK REPORT
Generated: ${timestamp}
Report ID: ${messageId}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
This report was generated by SatyaMatrix AI Fact-Checking System
For more information, visit: https://satyamatrix.com
            `.trim();
            ...
            a.href = url;
            a.download = `fact-check-report-${messageId}.txt`;
    ...
    // Publish fact-check function
    window.publishFactCheck = async function(messageId, buttonElement) {
        try {
            buttonElement.disabled = true;
            buttonElement.innerHTML = '<span>Publishing...</span>';
            
            const response = await fetch(`/auth/chat/api/messages/${messageId}/publish/`, {
                method: 'POST',
                headers: {
                    'X-CSRFToken': getCookie('csrftoken')
                }
            ...
            if (data.success) {
                ...
                buttonElement.innerHTML = `
                    ...
                    Published
                `;
                buttonElement.classList.add('published');
                
                // Add success message as a bot response
                addMessage('✅ Fact-check report published and emailed successfully!', 'bot');
            } else {
            ...
            addMessage('❌ Failed to publish: ' + error.message, 'bot');
        }
    }
    ...
    async function loadConversationHistory(conversationId) {
        try {
            ...
            if (data.success && data.messages.length > 0) {
                ...
                data.messages.forEach(msg => {
                    addMessage(msg.content, msg.sender, msg._id || msg.id);
                });
            }
        } catch (error) {
